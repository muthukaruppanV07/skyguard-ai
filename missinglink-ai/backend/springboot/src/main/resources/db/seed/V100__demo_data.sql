...
INSERT INTO users (email, password_hash, full_name, phone, role_id, status, mfa_enabled) VALUES
  ('admin@missinglink.local', '$2b$10$RLzJO7FpSedEvdrvfWf/wuTDgKgY/tGnkeBgpq7GHS.SjMvI6iKlG', 'Aisha Verma',   '+91-0000000001', (SELECT id FROM roles WHERE name='ADMIN'),         'ACTIVE', FALSE),
  ('investigator@example.com', '$2b$10$RLzJO7FpSedEvdrvfWf/wuTDgKgY/tGnkeBgpq7GHS.SjMvI6iKlG', 'Rohan Deshmukh', '+91-0000000002', (SELECT id FROM roles WHERE name='INVESTIGATOR'), 'ACTIVE', FALSE),
...
SELECT pm.id, (SELECT id FROM users WHERE email='investigator@example.com'),
...
WHERE s.case_id = c.id AND s.status <> 'REJECTED' AND u.email='investigator@example.com'
...
FROM missing_person_cases c, users u WHERE c.case_reference='MP-101' AND u.email='investigator@example.com';
...
FROM missing_person_cases c, sightings s, users u
WHERE c.case_reference='MP-102' AND s.sighting_reference='ST-1005' AND u.email='investigator@example.com';

INSERT INTO case_timeline (case_id, actor_id, event_type, description, payload, created_at)
SELECT c.id, u.id, 'STATUS_CHANGED', 'Case marked LOCATED.', jsonb_build_object('status','LOCATED'), now() - interval '12 days'
FROM missing_person_cases c, users u WHERE c.case_reference='MP-110' AND u.email='investigator@example.com';
